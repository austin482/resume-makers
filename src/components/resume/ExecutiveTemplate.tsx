    ...
    <div style={{ fontFamily: 'Georgia, serif', color: '#1a1a2e', fontSize: '10pt', lineHeight: 1.6 }}>
      ...
      <div style={{ background: '#1a1a2e', color: 'white', padding: '12mm 16mm 10mm' }}>
        ...
        <div style={{ display: 'flex', gap: '6mm', fontSize: '9pt', color: '#a0aec0', flexWrap: 'wrap' }}>
          {personalInfo.email && <InlineEdit placeholder="Email" value={`✉ ${personalInfo.email}`} onChange={(val) => useResumeStore.getState().setPersonalInfo({ email: val.replace(/^✉\s*/, '') })} />}
          ...
          {personalInfo.portfolioLine && <InlineEdit placeholder="Portfolio" value={`🌐 ${personalInfo.portfolioLine}`} onChange={(val) => useResumeStore.getState().setPersonalInfo({ portfolioLine: val.replace(/^🌐\s*/, '') })} />}
        </div>
      </div>
