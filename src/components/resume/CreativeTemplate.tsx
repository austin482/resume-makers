      ...
      <div style={{ width: '34%', backgroundColor: '#1e1b4b', color: '#c7d2fe', padding: '12mm 8mm', display: 'flex', flexDirection: 'column', gap: '8mm' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2mm', fontSize: '8.5pt' }}>
            ...
            <InlineEdit placeholder="Email" value={personalInfo.email ? `✉ ${personalInfo.email}` : ''} onChange={(val) => useResumeStore.getState().setPersonalInfo({ email: val.replace(/^✉\s*/, '') })} style={{ wordBreak: 'break-all' }} />
            <InlineEdit placeholder="Portfolio" value={personalInfo.portfolioLine ? `🌐 ${personalInfo.portfolioLine}` : ''} onChange={(val) => useResumeStore.getState().setPersonalInfo({ portfolioLine: val.replace(/^🌐\s*/, '') })} style={{ wordBreak: 'break-all' }} />
          </div>
        </div>
